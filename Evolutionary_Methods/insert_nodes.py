...
def insert_node_in_route(graph, route_from, route_to, max_vehicle_capacity, total_distance):
    ...
    if len(route_to) > 4:
        ...
        # Maneja el caso donde route_to no tiene cinco elementos, quizás inicializando original_distance_to a un valor por defecto
        original_distance_from  = 0  # O algún valor adecuado según el contexto
    
    if len(route_to) > 4:
        original_distance_to = route_to[4]
    else:
        # Maneja el caso donde route_to no tiene cinco elementos, quizás inicializando original_distance_to a un valor por defecto
        original_distance_to = 0  # O algún valor adecuado según el contexto
    ...
    for idx1, route1 in enumerate(routes):
        for idx2 in range(idx1 + 1, len(routes)):
            ...
            if improved1 or improved2:
                ...
                # Asegúrate de que cada ruta tiene al menos 5 elementos
                total_distance = sum(route[4] if len(route) > 4 else 0 for route in routes)
...
def main():
    for instance_filename in os.listdir(INSTANCES_DIR):
        if instance_filename.endswith('.txt'):
            ...
            #print('ROUTES:', routes)
            vehicles, total_distance, computation_time, routes, vehicle_capacity = insert_nodes(graph, vehicles, total_distance, computation_time, routes, vehicle_capacity)
